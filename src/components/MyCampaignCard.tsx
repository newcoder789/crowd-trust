...
export const MyCampaignCard: React.FC<MyCampaignCardProps> = ({ contractAddress }) => {
    ...
    const { data:campaignImage, isLoading: isLoadingImage } = useReadContract({
        contract,
    ...
    return (
            <div className="flex flex-col justify-between max-w-sm p-6 bg-white border border-slate-200 rounded-lg shadow">
                {campaignImage && (
            <img src={campaignImage} alt="Campaign" className="w-full h-64 object-cover rounded-lg mb-4" />
        )}
                <div>
